...
class RegistroBomberoSchema(Schema):
    ...
    cedula = fields.Str(required=True, validate=validate.Length(min=6, max=20))
...
class BomberoSchema(Schema):
    ...
    estado_aprobacion = fields.Str(
        validate=validate.OneOf(['pendiente', 'aprobado', 'rechazado'])
    )
...
class ActualizarPerfilBomberoSchema(Schema):
    ...
    direccion = fields.Str(validate=validate.Length(min=5, max=500))
    especialidades = fields.List(fields.Str())
    certificaciones = fields.List(fields.Str())
...
class AprobarBomberoSchema(Schema):
    """Schema para aprobar/rechazar bombero"""
    accion = fields.Str(required=True, validate=validate.OneOf(['aprobar', 'rechazar']))
    codigo_jefe = fields.Str(required=True, validate=validate.Length(min=3, max=50))
    password_jefe = fields.Str(required=True, validate=validate.Length(min=6))